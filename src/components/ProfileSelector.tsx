...
export default function ProfileSelector({ onProfileSelected, onOpenDashboard }: ProfileSelectorProps) {
  ...
  const handleCreate = () => {
    if (!name.trim()) return;
    const newProfile: Profile = { id: `profile_${Date.now()}`, name: name.trim(), level, avatar, createdAt: new Date().toISOString() };
  ...
  const handleSelect = (id: string) => { saveActiveProfile(id); onProfileSelected(id); };
      ...
      <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'linear-gradient(180deg,#F0F9FF,#F8FAFC)', padding: 24 }}>
        <div style={{ background: '#fff', borderRadius: 24, padding: 28, maxWidth: 400, width: '100%', boxShadow: '0 6px 24px rgba(0,0,0,0.06)' }}>
          ...
          <div style={{ display: 'flex', gap: 12, marginBottom: 16 }}>
            {LEVELS.map(l => (<button key={l} onClick={() => setLevel(l)} style={{ flex: 1, padding: 10, borderRadius: 12, border: level === l ? '2px solid #4F46E5' : '2px solid #E2E8F0', background: level === l ? '#EEF2FF' : '#fff', fontWeight: 700, color: level === l ? '#4F46E5' : '#64748B', cursor: 'pointer' }}>{l}</button>))}
          </div>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, justifyContent: 'center', marginBottom: 20 }}>
            {AVATARS.map(a => (<button key={a} onClick={() => setAvatar(a)} style={{ fontSize: 28, width: 48, height: 48, borderRadius: '50%', border: avatar === a ? '3px solid #4F46E5' : '2px solid #E2E8F0', background: avatar === a ? '#EEF2FF' : '#f8fafc', cursor: 'pointer' }}>{a}</button>))}
          </div>
    ...
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'linear-gradient(180deg,#F0F9FF,#F8FAFC)', padding: 24 }}>
      <div style={{ background: '#fff', borderRadius: 24, padding: 28, maxWidth: 400, width: '100%', boxShadow: '0 6px 24px rgba(0,0,0,0.06)' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
          ...
          <button onClick={onOpenDashboard} style={{ padding: '6px 12px', borderRadius: 8, background: '#F1F5F9', border: 'none', fontSize: 13, fontWeight: 600, color: '#475569', cursor: 'pointer' }}>👨‍👩‍ Parents</button>
        </div>

...
import PhoneticsApp from './PhoneticsApp'; // Import du nouveau module
...
export default function FrenchApp({ onBack }: FrenchAppProps) {
  ...
  if (showPhonetics) {
    return <PhoneticsApp onBack={() => setShowPhonetics(false)} />;
  }
    ...
    <div style={{ minHeight:'100vh', padding:'32px 16px', background:'linear-gradient(180deg,#F0F9FF,#F8FAFC)', position: 'relative' }}>
      ...
      <div style={{ maxWidth:560, margin:'0 auto', textAlign:'center' }}>
        ...
        <button 
          ...
          style={{ width:'100%', padding:16, marginBottom:20, borderRadius:20, background:'linear-gradient(135deg,#8B5CF6,#6D28D9)', color:'#fff', border:'none', fontWeight:800, fontSize:18, boxShadow:'0 4px 14px rgba(109,40,217,0.3)', display:'flex', alignItems:'center', justifyContent:'center', gap:12 }}
        >
          🗣️ Phonétique Montessori
        </button>

        {Object.keys(errCnt).length > 0 && (
          <button onClick={startSpecial} style={{ width:'100%', padding:16, marginBottom:20, borderRadius:20, background:'linear-gradient(135deg,#F59E0B,#D97706)', color:'#fff', border:'none', fontWeight:800, fontSize:18 }}>
      ...
      <div style={{ minHeight:'100vh', padding:32, background:'linear-gradient(180deg,#F0F9FF,#F8FAFC)', display:'flex', alignItems:'center', justifyContent:'center' }}>
        ...
        <div style={{ background:'#fff', borderRadius:28, padding:32, maxWidth:480, width:'100%', boxShadow:THEME.shadow, textAlign:'center' }}>
          ...
          <div style={{ display:'flex', gap:12 }}>
            ...
            <button onClick={() => setScreen('home')} style={{ flex:1, padding:16, borderRadius:20, background:'#64748B', color:'#fff', border:'none', fontWeight:800, fontSize:17 }}>🏠 Menu</button>
          </div>
        </div>
      </div>
    );

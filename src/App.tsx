...
function buildRounds(d: Dictée): Round[] {
  ...
  for (let i = 0; i < ROUNDS; i++) {
    const t = seq[i];
    if (t === 'qcm') { 
      out.push({ type: 'qcm', data: q[qi % q.length] }); 
      qi++; 
    }
    else if (t === 'blank' || t === 'tile') { 
      out.push({ type: t, phrase: p[pi % p.length], mots: d.mots }); 
      pi++; 
    }
    else { 
      out.push({ type: t, word: w[wi % w.length] }); 
      wi++; 
    }
  }
  return out;
  ...
  const onDone = (ok:boolean, w:string) => {
    ...
    if(next >= rounds.length) {
      ...
      setParentData(d => { 
        ...
        u.correct += ns; 
        u.lastDate = new Date().toLocaleDateString(); 
        saveParentStats(u); 
        return u; 
      });
      setScreen('result');
    ...
    <div style={{ minHeight:'100vh', padding:32, background:'#F1F5F9' }}>
      <div style={{ maxWidth:600, margin:'0 auto', background:'#fff', borderRadius:24, padding:28, boxShadow:THEME.shadow }}>
        ...
        <div style={{ display:'flex', flexWrap:'wrap', gap:8, marginBottom:24 }}>
          {Object.entries(errCnt).sort((a,b)=>b[1]-a[1]).map(([w,c]) => (
            <span key={w} style={{ background:'#FFF7ED', color:'#C2410C', padding:'6px 12px', borderRadius:16, fontWeight:600, fontSize:14 }}>{w} <small>({c}x)</small></span>
          ))}
        </div>
        <h3 style={{ color:'#475569' }}>📊 Dictées terminées :</h3>
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(120px, 1fr))', gap:10 }}>
          {DICTEES.map((d: Dictée) => { 
            const p = completed[d.id] || 0; 
            return (
              <div key={d.id} style={{ padding:12, background: p ? '#F0FDF4' : '#F8FAFC', borderRadius:14, textAlign:'center', border: `2px solid ${p ? '#86EFAC' : '#E2E8F0'}` }}>
                <div style={{ fontSize:16, fontWeight:700 }}>Dictée {d.id}</div>
                <div style={{ fontSize:14, color:'#059669' }}>{p ? '⭐'.repeat(Math.round(p/33.3)) : 'Non fait'}</div>
              </div>
            ); 
          })}
        </div>
    ...
    <div style={{ minHeight:'100vh', padding:'32px 16px', background:'linear-gradient(180deg,#F0F9FF,#F8FAFC)' }}>
      <div style={{ maxWidth:560, margin:'0 auto', textAlign:'center' }}>
        ...
        {Object.keys(errCnt).length > 0 && (
          <button onClick={startSpecial} style={{ width:'100%', padding:16, marginBottom:20, borderRadius:20, background:'linear-gradient(135deg,#F59E0B,#D97706)', color:'#fff', border:'none', fontWeight:800, fontSize:18, boxShadow:'0 4px 14px rgba(217,119,6,0.3)' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(140px,1fr))', gap:14 }}>
          {DICTEES.map((d: Dictée) => {
            ...
            return (
              <button 
                key={d.id} 
                onClick={()=>startWarm(d.id - 1)} 
                style={{ 
                  padding:20, borderRadius:24, 
                  border: `3px solid ${stars ? '#FCD34D' : '#E2E8F0'}`, 
                  background: stars ? '#FEF3C7' : '#fff', 
                  cursor:'pointer', 
    ...
    return (
      <div style={{ minHeight:'100vh', padding:32, background:'linear-gradient(180deg,#F0F9FF,#F8FAFC)', display:'flex', alignItems:'center', justifyContent:'center' }}>
        ...
        <div style={{ background:'#fff', borderRadius:28, padding:32, maxWidth:480, width:'100%', boxShadow:THEME.shadow, textAlign:'center' }}>
          ...
          {errs.length > 0 && (
            <div style={{ background:'#FFF7ED', borderRadius:20, padding:16, marginBottom:24, textAlign:'left' }}>
              <div style={{ fontWeight:700, color:'#C2410C', marginBottom:8 }}>📝 À revoir :</div>
              <div style={{ display:'flex', flexWrap:'wrap', gap:8 }}>
                {[...new Set(errs)].map(w => (
                  <span key={w} style={{ background:'#FED7AA', color:'#9A3412', padding:'6px 12px', borderRadius:16, fontWeight:600, fontSize:14 }}>{w}</span>
                ))}
              </div>
            </div>
          )}
          <div style={{ display:'flex', gap:12 }}>
            <button 
              onClick={() => {
                if (dicIdx !== null) startWarm(dicIdx);
                else startSpecial();
              }} 
              style={{ flex:1, padding:16, borderRadius:20, background:'#4F46E5', color:'#fff', border:'none', fontWeight:800, fontSize:17 }}
            >
              🔄 Rejouer
            </button>
            <button 
              onClick={() => setScreen('home')} 
              style={{ flex:1, padding:16, borderRadius:20, background:'#64748B', color:'#fff', border:'none', fontWeight:800, fontSize:17 }}
            >
              🏠 Accueil
            </button>
          </div>
    ...
    <div style={{ minHeight:'100vh', padding:'24px 16px', background:'linear-gradient(180deg,#F0F9FF,#F8FAFC)', display:'flex', flexDirection:'column', alignItems:'center' }}>
      ...
      <div style={{ width:'100%', maxWidth:540, marginBottom:16 }}>
        <div style={{ display:'flex', justifyContent:'space-between', fontSize:14, color:'#64748B', marginBottom:4 }}>
          <span>Jeu {ridx+1}/{rounds.length}</span>
        </div>
